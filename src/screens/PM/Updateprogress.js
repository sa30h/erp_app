...
const Updateprogress = ({navigation}) => {
    ...
    async function getTask(id) {
      ...
      await  Axios.get('http://196.29.238.98/api/pm/tasksAPI')
            .then((res)=>{setTask(res.data)
              setMytask(res.data.filter(function (item) {
                   return item.member == Number(id);}))
                              setIsloading(false)
...
async function getMember() {
  ...
  renderItem={({item})=>{return(
    <TouchableOpacity onPress={()=>navigation.navigate('Taskput',{id:item.id})}>
